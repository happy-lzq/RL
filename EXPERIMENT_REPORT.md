...
### 4.2 定性分析：动态演示 (GIF) 色变过程解读

![动态演示](demo.gif)

生成的 `demo.gif` 直观地展示了智能体的决策逻辑。以下是动画中**颜色变化与动作的对应意义**：

| 视觉元素 | 颜色/状态变化 | 物理意义与逻辑解读 |
| :--- | :--- | :--- |
| **左上角圆圈** | **金色 (Gold)** | **洋葱供应点**。智能体到达此处并执行“交互”，身上会出现黄色小点，表示“获取了食材”。 |
| **右上角方块** | **黑色 (Black)** | **空锅**。等待放入洋葱。 |
| **右上角方块** | **变橙色 (Orange)** | **烹饪中**。智能体放入洋葱后，锅进入倒计时状态。此时智能体通常会利用这段时间去拿盘子（并行处理）。 |
| **右上角方块** | **变红色 (Red)** | **烹饪完成**。汤已经煮好，必须用盘子才能盛出来。 |
| **左下角圆圈** | **白色 (White)** | **盘子供应点**。智能体在此处获取空盘子（身上出现白点）。 |
| **智能体持有物** | **白点 ➝ 红点** | **盛汤动作**。当持有盘子（白点）的智能体与红色的锅交互，盘子变红（盛满汤），锅变回黑色（空）。 |
| **右下角方块** | **紫色 (Purple)** | **上菜区**。持有红点（汤）的智能体到达此处交互，完成任务，Episode 结束。 |
